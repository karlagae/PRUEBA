...
def obtener_datos_acciones(etfs, start_date, end_date):
    datos = {}
    for etf in etfs:
        datos[etf] = yf.download(etf, start=start_date, end=end_date)['Adj Close']
...
# Convertir Q y P a matrices NumPy
try:
    Q = np.array(eval(Q_input))
    P = np.array(eval(P_input))
except:
    st.error("Por favor, ingrese Q y P en el formato correcto.")
